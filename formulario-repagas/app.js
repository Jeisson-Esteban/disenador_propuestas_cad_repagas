...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Si estamos en el formulario y existe el select de modelos, cargar la lista
  // de forma diferida al primer despliegue del <details> "Configuracion avanzada".
  const modelSelect = document.getElementById("openrouterModel");
  if (modelSelect) {
    const detailsEl = modelSelect.closest("details");
    let modelosCargados = false;
    const cargar = async () => {
      if (modelosCargados) return;
      modelosCargados = true;
      try {
        const resp = await fetch(getBaseUrl() + "/openrouter/modelos");
        if (!resp.ok) throw new Error("HTTP " + resp.status);
        const data = await resp.json();
        const defaultModel = data.default || "google/gemini-3-flash-preview";
        const modelos = Array.isArray(data.modelos) ? data.modelos : [];
        modelSelect.innerHTML = "";
        // Default primero (con etiqueta)
        const optDefault = document.createElement("option");
        optDefault.value = "";
        optDefault.textContent = `Por defecto (${defaultModel})`;
        ...
        console.warn("[OpenRouter] No se pudo cargar la lista de modelos:", e);
        modelSelect.innerHTML = '<option value="">Por defecto (google/gemini-3-flash-preview)</option>';
      }
    };
    if (detailsEl) {
      detailsEl.addEventListener("toggle", () => { if (detailsEl.open) cargar(); }, { once: false });
    } else {
      cargar();
    }
  }
...
function addEquipoRow(zone, nombre = "", cantidad = 1, descuentoActivo = true, descuentoPct = "") {
  ...
  row.innerHTML =
    ...
    '<label class="equipo-dto-toggle" title="Aplicar descuento a este equipo">' +
      '<input type="checkbox" class="equipo-dto-check"' + (descuentoActivo ? ' checked' : '') + ' />' +
      '<span>Dto</span>' +
    '</label>' +
    '<input type="number" class="equipo-dto-pct" min="0" max="100" step="0.5" placeholder="general" value="' + descuentoPct + '" title="% descuento (vacio = usar general)" />' +
  ...
  // Mostrar/ocultar input de % segun checkbox
  const check = row.querySelector(".equipo-dto-check");
  const pctInput = row.querySelector(".equipo-dto-pct");
  const syncPct = () => {
    pctInput.style.display = check.checked ? "" : "none";
    if (!check.checked) pctInput.value = "";
  };
  check.addEventListener("change", syncPct);
  syncPct();
...
function getManualEquipos(zone) {
  ...
  rows.forEach((row) => {
    ...
    const checkEl = row.querySelector(".equipo-dto-check");
    const pctEl = row.querySelector(".equipo-dto-pct");
    const descuento_activo = checkEl ? checkEl.checked : true;
    let descuento_pct = null;
    if (descuento_activo && pctEl && pctEl.value !== "") {
      const v = parseFloat(pctEl.value);
      if (!isNaN(v)) descuento_pct = Math.max(0, Math.min(100, v));
    }
    result.push({ nombre, cantidad, descuento_activo, descuento_pct });
  });
  return result;
}
...
function getBaseUrl() {
  return "https://autocad-generador-de-planos-propuestas-production.up.railway.app";
}
...
function buildFormularioJSON() {
  ...
  return {
    ...
    observaciones_pasos: recolectarObservacionesPasos(),
  };

  // Si el cliente ha rellenado equipos en el paso 5 (importados desde Excel o
  // anadidos manualmente, cada uno con su descuento), esos son los equipos
  // VINCULANTES para el backend. Los volcamos en necesidades_equipamiento (que
  // es lo que el schema Pydantic del backend lee) sobreescribiendo el contenido
  // del paso 4 (texto suelto), ya que el paso 5 es mas especifico.
  const eqm = data.equipos_manuales;
  const totalManual =
    (eqm.coccion?.length || 0) +
    (eqm.refrigeracion?.length || 0) +
    (eqm.lavado?.length || 0) +
    (eqm.horno?.length || 0);
  if (totalManual > 0) {
    const nombres = (arr) => (arr || []).map((e) => e.nombre).filter(Boolean);
    data.necesidades_equipamiento.coccion = nombres(eqm.coccion);
    data.necesidades_equipamiento.refrigeracion = nombres(eqm.refrigeracion);
    data.necesidades_equipamiento.lavado = nombres(eqm.lavado);
    // necesidades_equipamiento no tiene zona "horno" en el schema del backend,
    // pero _construir_desde_lista_cliente detecta hornos por nombre y los
    // reubica solos en zona_horno. Los metemos en "otros" para que pasen.
    data.necesidades_equipamiento.otros = nombres(eqm.horno);
  }

  return data;
}

function recolectarObservacionesPasos() {
  const obs = {};
  for (let s = 1; s <= 9; s++) {
    const el = document.getElementById(`obs_paso_${s}`);
    if (el && el.value.trim()) obs[`paso_${s}`] = el.value.trim();
  }
  return obs;
}
...
form.addEventListener("submit", async (e) => {
  ...
  const incluirDxf = document.getElementById("incluirDxf")?.checked || false;
  const descuentoRaw = document.getElementById("descuentoGeneral")?.value;
  const descuento = descuentoRaw ? Math.max(0, Math.min(100, parseFloat(descuentoRaw) || 0)) : 0;
  const openrouterModel = document.getElementById("openrouterModel")?.value || "";

  // Construir descuentos individuales y lista de modelos sin descuento
  // a partir de los checkboxes/inputs del paso 5
  const descuentosIndividuales = {};
  const sinDescuento = [];
  ["coccion", "refrigeracion", "lavado", "horno"].forEach((zone) => {
    (data.equipos_manuales?.[zone] || []).forEach((eq) => {
      if (!eq.descuento_activo) {
        sinDescuento.push(eq.nombre);
      } else if (eq.descuento_pct != null) {
        descuentosIndividuales[eq.nombre] = eq.descuento_pct;
      }
    });
  });
  ...
  try {
    ...
    if (hasFile) {
      ...
      formData.append("incluir_dxf", incluirDxf ? "true" : "false");
      formData.append("descuento_general", String(descuento));
      formData.append("descuentos_individuales", JSON.stringify(descuentosIndividuales));
      formData.append("sin_descuento", JSON.stringify(sinDescuento));
      if (openrouterModel) formData.append("openrouter_model", openrouterModel);
      ...
      response = await fetch(baseUrl + "/generar", {
        ...
        body: JSON.stringify({
          ...data,
          _incluir_dxf: incluirDxf,
          _descuento_general: descuento,
          _descuentos_individuales: descuentosIndividuales,
          _sin_descuento: sinDescuento,
          ...(openrouterModel ? { _openrouter_model: openrouterModel } : {}),
        }),
      });
    }
...
document.getElementById("importExcelFile").addEventListener("change", async (e) => {
  ...
  if (statusEl) {
    statusEl.textContent = "Analizando Excel con IA (puede tardar 10-30s)...";
    statusEl.className = "import-status excel-status processing";
  }
  btn.disabled = true;
  ...
  let usadoIA = false;
  try {
    // Primero intentar con IA (mas robusto para cualquier formato)
    const fd = new FormData();
    fd.append("archivo", file);
    console.log("[Excel] Enviando a", getBaseUrl() + "/parsear-excel");
    const resp = await fetch(getBaseUrl() + "/parsear-excel", { method: "POST", body: fd });
    console.log("[Excel] Respuesta:", resp.status);
    if (!resp.ok) {
      const errTxt = await resp.text();
      console.warn("[Excel] Backend error:", errTxt);
      throw new Error("Backend " + resp.status);
    }
    const data = await resp.json();
    console.log("[Excel] Datos recibidos:", data);
    const added = aplicarEquiposImportados(data);
    const total = Object.values(added).reduce((s, n) => s + n, 0);
    usadoIA = true;

    if (total > 0) {
      if (statusEl) {
        statusEl.textContent = "Excel (IA): " + total + " equipos cargados desde " + file.name;
        statusEl.className = "import-status excel-status ok";
      }
      if (currentStep < 5) showStep(5);
      return;
    }
    // Si la IA devolvio 0 equipos, caer al parser local
    console.warn("[Excel] IA devolvio 0 equipos, intentando parseo local");
  } catch (err) {
    console.warn("[Excel] IA fallo:", err.message);
  }

  // Fallback: parseo local con SheetJS
  try {
    ...
    if (statusEl) {
      if (total > 0) {
        statusEl.textContent = "Excel: " + total + " equipos cargados (" + file.name + ")";
        statusEl.className = "import-status excel-status ok";
      } else {
        statusEl.textContent = "No se detectaron equipos. Revisa que el Excel tenga cabeceras tipo 'ZONA COCCION', 'ZONA LAVADO', etc.";
        statusEl.className = "import-status excel-status err";
      }
    }
    if (total > 0 && currentStep < 5) showStep(5);
  } catch (fallbackErr) {
    console.error("[Excel] Parseo local fallo:", fallbackErr);
    if (statusEl) {
      statusEl.textContent = "Error: no se pudo leer el archivo. " + fallbackErr.message;
      statusEl.className = "import-status excel-status err";
    }
  } finally {
    btn.disabled = false;
...
function aplicarEquiposImportados(data) {
  ["coccion", "refrigeracion", "lavado", "horno"].forEach((zone) => {
    const list = document.getElementById("equipos" + capitalize(zone) + "List");
...
// Normaliza texto para matching de zonas: mayusculas y sin tildes.
// Acepta tanto "COCCIÓN" precomposed como "COCCION" descomposed o sin tilde.
function _normalizarParaZona(s) {
  // NFD descompone "O acentuada" en "O" + U+0301 (combining acute). Luego
  // quitamos el rango U+0300-U+036F (Combining Diacritical Marks).
  return String(s)
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .toUpperCase()
...
function importEquiposFromExcelRows(rows) {
  ...
  // Mapa normalizado de palabras-zona (precalculado).
  const zonaMap = EXCEL_ZONA_MAP.map(({ key, zone }) => ({
    key: _normalizarParaZona(key),
    zone,
  }));

  console.log("[Excel] Filas a procesar:", rows.length);

  for (const row of rows) {
    if (!Array.isArray(row)) continue;

    // Filtrar celdas no nulas (texto o numero) y separar en texto vs numero
    const textCells = [];
    const numCells = [];
    for (const cell of row) {
      ...
      if (typeof cell === "number") {
        ...
        if (t) textCells.push(t);
      }
    }
    if (textCells.length === 0) continue;

    // 1) Detectar cabecera de zona: buscar palabra clave en CUALQUIER celda de
    //    texto. Usamos word boundaries (\b) para evitar falsos positivos como
    //    "LAVADO" dentro de "PRELAVADO" (el equipo "MESA PRELAVADO" no es
    //    cabecera de zona LAVADO).
    let isHeader = false;
    for (const text of textCells) {
      const textNorm = _normalizarParaZona(text);
      for (const { key, zone } of zonaMap) {
        const re = new RegExp(`\\b${key}\\b`);
        if (re.test(textNorm)) {
          currentZone = zone; // null = zona ignorada (barra, etc.)
          isHeader = true;
          console.log("[Excel] Cabecera detectada:", text, "-> zona:", zone);
          break;
        }
      }
      if (isHeader) break;
    ...
    // 2) Identificar el nombre del equipo: la celda de texto mas larga
    //    (descarta "UNIDADES" y similares que son cabeceras de columna)
    const candidates = textCells.filter((t) => {
      const upper = t.toUpperCase();
      return upper !== "UNIDADES" && upper !== "CANTIDAD" && upper !== "UDS" && t.length > 2;
    });
    if (candidates.length === 0) continue;
    // Elegir el texto mas largo (suele ser el nombre del equipo)
    const rawName = candidates.reduce((a, b) => (b.length > a.length ? b : a));

    // Quitar prefijo "1.01 ", "2.03 ", "01 ", etc.
    const codeMatch = rawName.match(/^\d+(?:\.\d+)?\s+([\s\S]+)/);
    const equipName = (codeMatch ? codeMatch[1] : rawName).trim();
    if (!equipName) continue;

    // 3) Cantidad: primer numero que aparezca en la fila (>=1)
    const qty = numCells.length > 0
      ? Math.max(1, Math.round(numCells[0]))
      : 1;
...
function fillFormFromJSON(data) {
  ...
  ["coccion", "refrigeracion", "lavado", "horno"].forEach((zone) => {
    ...
    if (Array.isArray(eqm[zone])) {
      eqm[zone].forEach((eq) => addEquipoRow(
        zone,
        eq.nombre,
        eq.cantidad || 1,
        eq.descuento_activo !== false,  // por defecto true
        eq.descuento_pct != null ? eq.descuento_pct : ""
      ));
    }
  });
...
// Se inyectan textareas de observaciones al final de cada paso (1-9).
// El paso 10 es resumen+envio, no necesita observaciones.

(function injectarObservaciones() {
  const titulos = {
    1: "Observaciones del proyecto",
    2: "Observaciones tecnicas del local",
    3: "Observaciones de energia/instalaciones",
    4: "Observaciones de equipamiento",
    5: "Observaciones de equipos manuales",
    6: "Observaciones de la cocina",
    7: "Observaciones de lavado",
    8: "Observaciones de refrigeracion",
    9: "Observaciones de personal y escalabilidad",
  };
  for (let s = 1; s <= 9; s++) {
    const section = document.querySelector(`.step[data-step="${s}"]`);
    if (!section) continue;
    if (section.querySelector(".observaciones-paso")) continue;  // ya inyectado
    const div = document.createElement("div");
    div.className = "field observaciones-paso";
    div.style.marginTop = "24px";
    div.style.paddingTop = "18px";
    div.style.borderTop = "1px solid var(--gray-200)";
    div.innerHTML = `
      <label style="color:var(--gray-600);font-weight:500">${titulos[s]} <span style="color:var(--gray-400);font-weight:400">(opcional)</span></label>
      <textarea id="obs_paso_${s}" class="field-textarea" rows="2"
                placeholder="Cualquier detalle adicional sobre este apartado que quieras compartir..."
                style="width:100%;padding:10px 12px;border:1px solid var(--gray-200);border-radius:8px;font-family:inherit;font-size:0.88rem;background:var(--gray-50);resize:vertical"></textarea>
    `;
    section.appendChild(div);
  }
})();

// ─── Auto-guardado + boton guardar ──────────────────────
// El progreso del formulario se guarda en localStorage cada 2s de inactividad
// y al cambiar de paso. Se restaura al abrir el formulario.

const DRAFT_KEY = "repagas_form_draft";
...
function saveDraft() {
  try {
    ...
    for (let s = 1; s <= 9; s++) {
      const el = document.getElementById(`obs_paso_${s}`);
      if (el && el.value.trim()) observaciones[`paso_${s}`] = el.value;
    }
    data.observaciones_pasos = observaciones;
    data._draft_updated = new Date().toISOString();
    data._draft_step = currentStep;
    localStorage.setItem(DRAFT_KEY, JSON.stringify(data));
    const indicator = document.getElementById("draftIndicator");
    if (indicator) {
      indicator.textContent = "Guardado " + new Date().toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });
...
function loadDraft() {
  try {
    ...
    if (!data.proyecto?.nombre && !data.proyecto?.tipo_negocio) return false;
    if (!confirm("Hay un borrador guardado de un formulario anterior. ¿Quieres continuar donde lo dejaste?")) {
      localStorage.removeItem(DRAFT_KEY);
      return false;
    }
    fillFormFromJSON(data);
    const obs = data.observaciones_pasos || {};
    Object.entries(obs).forEach(([k, v]) => {
      const num = k.replace("paso_", "");
      const el = document.getElementById(`obs_paso_${num}`);
      if (el) el.value = v;
    });
    if (data._draft_step && data._draft_step > 1) {
      showStep(Math.min(data._draft_step, TOTAL_STEPS));
    }
    return true;
  } catch (e) {
...
// Auto-guardar cada 2 segundos de inactividad tras cualquier cambio
let saveTimer = null;
...
// Guardar al navegar entre pasos y antes de cerrar la pagina
window.addEventListener("beforeunload", () => { saveDraft(); });

// Inyectar indicador de guardado y boton manual
(function injectarIndicadorGuardado() {
  const navButtons = document.querySelector(".nav-buttons");
  if (!navButtons || document.getElementById("draftIndicator")) return;
  const wrap = document.createElement("div");
  wrap.style.cssText = "display:flex;align-items:center;gap:14px;margin-top:14px;padding-top:12px;border-top:1px solid var(--gray-100);font-size:0.82rem;color:var(--gray-500)";
  wrap.innerHTML = `
    <span id="draftIndicator">Auto-guardado activado</span>
    <button type="button" id="btnGuardarManual" class="btn btn-secondary" style="padding:6px 14px;font-size:0.82rem">Guardar borrador</button>
    <button type="button" id="btnLimpiarDraft" class="btn btn-secondary" style="padding:6px 14px;font-size:0.82rem">Limpiar</button>
  `;
  navButtons.parentElement.appendChild(wrap);
  document.getElementById("btnGuardarManual").addEventListener("click", () => {
    saveDraft();
    const ind = document.getElementById("draftIndicator");
    ind.textContent = "Borrador guardado manualmente";
    ind.classList.add("saved");
  });
  document.getElementById("btnLimpiarDraft").addEventListener("click", () => {
    if (confirm("Borrar el borrador guardado? Esta accion no se puede deshacer.")) {
      clearDraft();
      document.getElementById("draftIndicator").textContent = "Borrador eliminado";
    }
  });
})();

// Al hacer submit exitoso se limpia el draft (se hace desde el handler de /generar)
