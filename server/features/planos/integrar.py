...
import ezdxf
from features.planos.analizar import EspacioCocina
from features.planos.posicionar import EquipoPosicionado


_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
LIBRERIA_DXF = os.path.join(_BASE_DIR, "..", "demo_assets", "demo_blocks.dxf")
BLOQUE_MAP_JSON = os.path.join(_BASE_DIR, "..", "demo_assets", "bloque_map.json")
...
def generar_dxf_catalogo(
    ...
    # recover.readfile repara DXFs danados (p.ej. generados por LibreDWG)
    try:
        from ezdxf import recover
        doc, auditor = recover.readfile(espacio.source_path)
        if auditor.has_errors:
            print(f"[CATALOGO] Auditor reparo {len(auditor.fixes)} problemas en el DXF")
    except Exception as e:
        print(f"[CATALOGO] recover.readfile fallo ({e}), usando readfile normal")
        doc = ezdxf.readfile(espacio.source_path)
    msp = doc.modelspace()

    # Asegurar que la tabla materials tiene entradas validas (evita crash de LibreDWG->DXF)
    try:
        materials = doc.materials
        for nombre in ("ByLayer", "ByBlock", "Global"):
            if nombre in materials:
                entry = materials.get(nombre)
                if isinstance(entry, str) or not hasattr(entry, "dxf"):
                    del materials[nombre]
                    print(f"[CATALOGO] Material corrupto '{nombre}' eliminado")
    except Exception as e:
        print(f"[CATALOGO] WARN limpiando materials: {e}")

    # LibreDWG a veces convierte los layers con flag OFF/FROZEN. Forzar ON para
    # que el renderer PNG dibuje el plano del cliente.
    layers_corregidos = 0
    try:
        for layer in doc.layers:
            if not layer.is_on() or layer.is_frozen():
                ...
                layer.thaw()
                layers_corregidos += 1
        if layers_corregidos:
            print(f"[CATALOGO] {layers_corregidos} layers reactivados (estaban OFF/FROZEN)")
    except Exception as e:
        print(f"[CATALOGO] WARN reactivando layers: {e}")
    ...
    try:
        ...
        # Build de fuentes: si el sistema no tiene fuentes OpenType, desactivar
        # el rendering de texto en el PNG para no fallar toda la generacion.
        try:
            from ezdxf import fonts as _fonts
            _fonts.font_manager.build(rebuild=False)
            has_fonts = len(_fonts.font_manager.font_cache) > 0
        except Exception:
            has_fonts = False
        ...
        if has_fonts:
            ...
            # Sin fuentes: renderizar con config que evita crash en textos
            config = Configuration(min_dash_length=0.1)
            try:
                Frontend(ctx, out, config=config).draw_layout(msp)
            except Exception:
                # Si aun falla, intentar sin textos
                for t in list(msp.query("TEXT MTEXT")):
                    try: msp.delete_entity(t)
...
def _dibujar_tabla_especificaciones(
    ...
    """
    Crea un BLOQUE insertable 'TABLA_EQUIPOS' con la tabla de especificaciones
    y lo inserta en el modelspace. Al ser bloque, el profesional puede
    seleccionarlo, moverlo, copiarlo o exportarlo como una unidad.
    """
    bx1, by1, bx2, by2 = espacio.boundary_rect
    ...
    # Crear el bloque con coordenadas locales (origen 0,0) — facil de mover en CAD
    block_name = "TABLA_EQUIPOS"
    if block_name in doc.blocks:
        del doc.blocks[block_name]
    block = doc.blocks.new(name=block_name)

    # Calcular punto de insercion en el modelspace
    try:
        from ezdxf import bbox as _bbox
        cache = _bbox.extents(msp)
    ...
    insert_x = bx1
    insert_y = min(by1, real_min_y) - 3000

    # Coordenadas locales del bloque (origen 0,0); se inserta al final
    table_x = 0
    table_y = 0
    msp_real = msp
    msp = block
    ...
    # Columnas: Pos | Descripcion | Uds | Marca | Dimensiones | Observaciones
    COL_WIDTHS = [600, 3800, 600, 1500, 1500, 3500]
    COL_HEADERS = ["POS.", "DESCRIPCION", "UDS", "MARCA", "DIMENSIONES", "OBSERVACIONES"]
    ...
    for zona in zonas_ordenadas:
        ...
        for i, ep in enumerate(items):
            ...
            observacion = _observacion_por_modelo(modelo)
            valores = [str(num), modelo, str(cant), marca, dims, observacion]
    ...
    msp_real.add_blockref(
        block_name,
        (insert_x, insert_y),
        dxfattribs={"layer": "tabla"},
    )

    print(f"[CATALOGO] Tabla de especificaciones (bloque {block_name}): "
          f"{num} filas en {len(zonas_ordenadas)} zonas")
...
# Reglas de observaciones por prefijo de modelo (definidas por cliente RepagasConcept).
# Cada tupla: (lista_de_prefijos_o_modelos_exactos, texto_observacion).
# Se evalua en orden; primer match gana. Los modelos EXACTOS (con "==") tienen prioridad
# sobre los prefijos.
_REGLAS_OBSERVACIONES = [
    # Modelos exactos primero (mayor especificidad)
    (["==E-64", "==E-84"], "Conexion electrica 220V III+N+T. Clavija domestica."),
    (["==FG-92/16", "==FG-72/16"], "Toma de gas de 1/2. 2 Tomas parte trasera."),
    (["==PG-600", "==PG-800", "==PG-640", "==PG-840"],
     "Toma de gas de 1/2. Parte trasera derecha del aparato."),
    # Prefijos especificos
    (["CG-11"], "Toma de Gas de 1/2. 2 tomas una a cada lado. Parte inferior."),
    (["FGT-"], "Toma de gas de 3/4 parte inferior. Pata delantera izquierda."),
    (["MG-"], "Toma de gas de 1/2 parte inferior. Pata delantera izquierda."),
    (["FES-"], "Conexion electrica 220V I+N+T."),
    # Prefijos de gas generales
    (["CG-5", "CG-7", "CG-9", "FTG-7", "FTG-9",
      "PG-4", "PG-6", "PG-9", "PG-12",
      "BARG-7", "BARG-9", "FG-71/16", "FG-91/16"],
     "Toma de Gas de 1/2. Parte trasera izquierda del aparato."),
    # Prefijos electricos (al final para no solapar con FES-, FTE-, etc. ya definidos)
    (["E-", "CE-", "FTE-", "ME-", "CPE-", "SBE-", "FE-"],
     "Conexion electrica 380V III+N+T. Clavija CETAC."),
]


def _observacion_por_modelo(modelo: str) -> str:
    """
    Devuelve la observacion tecnica (toma de gas / conexion electrica) para un modelo.
    Las reglas son definidas por el cliente (ver _REGLAS_OBSERVACIONES).
    """
    if not modelo:
        return ""
    nombre = modelo.strip().upper()
    for claves, texto in _REGLAS_OBSERVACIONES:
        for k in claves:
            if k.startswith("=="):
                if nombre == k[2:].upper():
                    return texto
            elif nombre.startswith(k.upper()):
                return texto
    return ""
