...
from features.planos.analizar import EspacioCocina, SegmentoPared
...
def _cargar_patrones() -> list[dict]:
    ...
    json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "demo_assets", "patrones_profesionales.json")
...
def _cargar_bloque_map() -> dict:
    ...
    json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "demo_assets", "bloque_map.json")
    if not os.path.exists(json_path):
        _BLOQUE_MAP_CACHE = {}
        return {}
    try:
...
def _buscar_alternativa_menor(
    ...
    """
    try:
        from features.propuestas.resolver import buscar_equipo_por_tipo
